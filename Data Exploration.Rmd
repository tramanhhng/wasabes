...
As provided by the client, the orientation of the hand-held device going in increaments of 45 degrees. We plotted 
...
The heatmap of position x, position y and number of observations is depicted bellow. We observe that the observations are not equally distributed.  We confirm that the locations overlap with the floormap.
```{r}
mod_OffLine %>% ggplot(aes(posX, posY)) + geom_bin_2d() + labs(title="Heatmap of Observations by Location", x="x-coordinate", y="y-coordinate") + scale_fill_gradient() + theme_minimal()
  
#   geom_count() + labs(title="Location map of access points and handheld devices",x="x-coordinate",y="y-coordinate")
# 
# toPlot2

```

```{r fig.align = "center", fig.cap= 'Flooplan location and offline data location. We see the data cleaning was sucessful in eliminating unwanted variables while preserving the geographical information', out.width="50%"}
