...
Since the model is supposed to rely on wifi signal strength to predict device location, we format the data so each observation corresponds to one `signal` value. We do this by performing the following operations:
 
* First, we remove the rows with the "#" symbol that mark the beggining of a location sampling/orientation sample. As a result, a total of 5312 rows are removed, resulting in 146,080 rows.  
...
Before further exploring and analyzing the data, we conduct a simple assessment to convert variables into the correct types. We perform the following operations:
...
* Based on the documents received from the client, a value of 1 for the variable `type` corresponds to ad-hoc devices. However, for the development and testing of the IPS, we will utilize only the signals measured at fixed access points. We removed all rows that have `type = 1`. This operation removes `length(clean_offline$time) - length(OffLine$time)` observations, resulting in `length(OffLine$time)` observations for all access point devices. Now, since the variable `type` has a value of 3 for all observations, so we eliminate the variable `type` from the data set.
...
sacheck = OffLine %>% group_by(mac) %>% summarize(observations = n())
...
OffLine = subset(OffLine, select = -channel)
