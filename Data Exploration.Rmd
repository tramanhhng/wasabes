...
Our client stores their data in a .txt file. This sections documents our steps to transfer the text data into a data frame format so that it is ready for further cleaning and analysis. 

The first six rows of the original, raw data is printed below.
...
Since our model is supposed to rely on wifi signal strength to predict device location, we need to format our data so that each observation corresponds to one single `signal` value. To transfer the data into a dataframe, we performed the following operations:   
...
Before further exploring and analyzing the data, we conduct a simple assessment to convert variables into the correct useful types and formats. We performed the following operations:
...
* Based on the documents received from the client, a value of 1 for the variable `type` corresponds to ad-hoc devices. However, for the development and testing of the IPS, we will utilize only the signals measured at fixed access points. We removed all rows that have `type` = 1. This operation removes `r length(clean_offline) - length(OffLine)` observations, resulting in `r length(OffLine)` observations for all accesspoint devices. Now, since the variable `type` has a value of 3 for all observations, so we eliminate the variable `type` from the data set.

Table 1 provides our formatted offline dataset.
...
kbl(head(clean_offline), caption = "Clean Data with transformed variables", position="b") %>% kable_classic(full_width = F, html_font = "Cambria", font_size = 5)

```

# 3 Data exploration
In this section, we explore the data and prepare it to be ready for model input.
...
#Filter out the mac addresses with the bottom five number of observations
OffLine_7macs <- OffLine %>% filter(!(mac %in% c("00:0f:a3:39:e0:4b",
                                                 "00:0f:a3:39:e2:10",
                                                 "00:04:0e:5c:23:fc",
                                                 "00:30:bd:f8:7f:c5",
                                                 "00:e0:63:82:8b:a9") ))
...
OffLine_7macs %>% 
  group_by(mac, channel) %>%
  summarize ()

#Dropping the channel variable in the data set
OffLine_7macs = subset(OffLine_7macs, select = -channel)
